...
const DataQuality = () => {
    ...
    useEffect(() => {
        getDataStats("data_stats_1",accessToken, refreshToken).then(res => {
            if (res) {
        ...
        getDataStats("data_stats_2",accessToken, refreshToken).then(res => {
            if (res) {
               ...
               let heatmapData = [
                    ...
                    ["USA", null,0],
                    ...responseData.map(entry => [entry.State,"USA", entry.Distinct_Patients])
                  ];
        ...
        getDataStats("data_stats_7",accessToken, refreshToken).then(res => {
            if (res) {
                const responseData = res.data
  ...
  return (
    <div className='pb-44'>
       {statsData1 && <LineChart arbitrary={false} data={statsData1} options={options} />}
       {statsData2 && <TreeMap needCallbacks={false} data={statsData2} />}
       {statsData3 && <Table Title="Asthma HCP Specialties by Patient Interactions" activeCells={false} showSelectionBtns={false} TableData={statsData3} TableColummns={TableColummns} />}
    </div>
  )
}
