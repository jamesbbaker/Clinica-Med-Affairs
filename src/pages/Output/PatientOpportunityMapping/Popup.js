...
const BarChartPopup = ({ data1 }) => {
  ...
  useEffect(() => {
    if (data1) {
      console.log("first", data1[0]["Provider ID"])
      getDataStats(`hcp_quarterly?Provider_ID=${data1[0]["Provider ID"]}`, accessToken, refreshToken)
        .then((res) => {
          console.log(res, "respnse")
          // let response = JSON.parse(res.replaceAll(NaN, 0));
