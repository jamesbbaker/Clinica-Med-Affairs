...
const TargetList = () => {
  ...
  const [selectableChart, setSelectableChart] = useState([]);
  ...
  useEffect(() => {
    if (selectableChart.length > 0 && selectedChart) {
      setLoading(true);
      getDataStats(
    ...
    } else if (selectableChart.length > 0 && selectedChart === null) {
      setSelectedChart(selectableChart[0]);
    }
  }, [selectableChart, selectedChart]);
    ...
    getDataStats("get_list", accessToken, refreshToken)
      .then((res) => {
        if (res.data) {
          ...
          setSelectableChart([...new Set(res.data.map((item) => item.type))]);
          setFilters(_data);
        }
      })
      .catch((err) => {
    ...
    <>
      {filters ? (
        <div className="flex flex-col items-start gap-4">
          ...
          {selectableChart && (
            <div className="flex flex-col items-start mt-10 gap-2 w-full">
              <CustomDropdown
                buttonWidth="40rem"
                labelClassName="mb-0"
                ...
                input={{
                  ...
                  name: "Select Chart",
                  type: "select",
                  options: selectableChart.map((item) => ({
                    name: item,
                    value: item,
                  })),
                  id: "xLabel",
